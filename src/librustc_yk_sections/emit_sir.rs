...
fn do_generate_sir<'a, 'tcx, 'gcx>(
    tcx: &'a TyCtxt<'a, 'tcx, 'gcx>, mut def_ids: DefIdSet, mode: &SirMode)
    -> Result<PathBuf, Box<dyn Error>>
{
    ...
    // Sort the DefIds to satisfy reproducible build tests.
    let mut def_ids = def_ids.drain().collect::<Vec<DefId>>();
    def_ids.sort();
