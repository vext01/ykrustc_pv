...
struct ConvCx<'a, 'tcx, 'gcx> {
    ...
    next_tir_var: Cell<ykpack::LocalIndex>,
    /// A mapping from MIR variables to TIR variables.
    var_map: RefCell<IndexVec<Local, Option<ykpack::Local>>>,
...
impl<'a, 'tcx, 'gcx> ConvCx<'a, 'tcx, 'gcx> {
    ...
    fn new_tir_var(&self) -> ykpack::LocalIndex {
    ...
    fn tir_var(&self, local: Local) -> ykpack::Local {
